...
function Start-Installation {
    ...
    $packageManager = $null
    if ($needsNode -or $needsGit -or $needsGitHubDesktop -or $needsCode) {
    ...
    if ($needsGitHubDesktop) {
        Write-Host "`nStep 4.5: Installing GitHub Desktop..." -ForegroundColor Cyan

        $ghDesktopInstalled = $false
        ...
        if ($packageManager -eq 'winget') {
            ...
            $ghDesktopInstallSuccess = ($LASTEXITCODE -eq 0)

            if ($ghDesktopInstallSuccess) {
                Write-Host "  [OK] GitHub Desktop installed successfully via winget" -ForegroundColor Green
                $ghDesktopInstalled = $true
            } else {
                Write-Host "  [WARNING] winget installation failed, trying Chocolatey..." -ForegroundColor Yellow
            }
        }

        if (-not $ghDesktopInstalled -and $packageManager -eq 'choco') {
            Write-Host "  Installing GitHub Desktop via Chocolatey..." -ForegroundColor Yellow
            choco install github-desktop -y
            $ghDesktopInstallSuccess = ($LASTEXITCODE -eq 0)

            if ($ghDesktopInstallSuccess) {
                Write-Host "  [OK] GitHub Desktop installed successfully via Chocolatey" -ForegroundColor Green
                $ghDesktopInstalled = $true
            } else {
        ...
        if ($ghDesktopInstalled) {
            ...
            if (Test-GitHubDesktop -Verbose) {
                Write-Host "`n  [OK] GitHub Desktop is ready to use!" -ForegroundColor Green
            } else {
                Write-Host "`n  [WARNING] GitHub Desktop was installed but cannot be verified" -ForegroundColor Yellow
                Write-Host "  You may need to restart your computer for changes to take effect" -ForegroundColor Yellow
            }
        } else {
            Write-Host "`n  [WARNING] GitHub Desktop installation failed" -ForegroundColor Yellow
            Write-Host "  You can manually install it from: https://desktop.github.com/" -ForegroundColor Yellow
            Write-Host "  Continuing with remaining installations..." -ForegroundColor Cyan
        }
    }
    ...
    if ($needsPiecesDesktop) {
        ...
        if ($packageManager -eq 'winget') {
            ...
            try {
                ...
                # Using pattern from install-pieces-v.1.ps1 (industry-standard)
                ...
                Write-Host ""
                Write-Host "[OK] Pieces Desktop installed successfully from Microsoft Store!" -ForegroundColor Green
            ...
            # Chocolatey doesn't have Pieces Desktop - direct user to website
            Write-Host "[INFO] Pieces Desktop is not available via Chocolatey" -ForegroundColor Yellow
            Write-Host ""
            Write-Host "To install Pieces Desktop manually:" -ForegroundColor Cyan
            Write-Host "  1. Visit: https://pieces.app" -ForegroundColor Gray
            Write-Host "  2. Download the installer for Windows" -ForegroundColor Gray
            Write-Host "  3. Run the downloaded installer" -ForegroundColor Gray
            Write-Host ""
            Write-Host "Or install via Microsoft Store:" -ForegroundColor Cyan
            Write-Host "  - Search for 'Pieces for Developers' in Microsoft Store" -ForegroundColor Gray
            Write-Host ""
            $piecesInstallSuccess = $false
        }
