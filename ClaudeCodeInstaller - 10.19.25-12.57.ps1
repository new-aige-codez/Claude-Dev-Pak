...
# ===================================================================
# Global error action preference - continue on errors but track them
...
trap {
    ...
    Write-Host ""
    Write-Host "[INFO] The installer will attempt to continue..." -ForegroundColor Cyan
    Write-Host "       Check the installation log for details." -ForegroundColor Cyan
    Write-Host ""
                ...
                if ($majorVersion -ge 25) {
                    ...
                    if ($recoveryChoice -eq 'r') {
                        ...
                        if (-not $recoveryNodeSuccess) {
                            Write-Host "[ERROR] Failed to install Node.js 20 LTS during recovery" -ForegroundColor Red
                            Read-Host "Press Enter to exit"
                            exit 1
                        }
...
"@
        ...
        if (-not $vscodeInstallSuccess) {
            Write-Host "[ERROR] Failed to install Visual Studio Code" -ForegroundColor Red
            Read-Host "Press Enter to exit"
            exit 1
        } else {
    ...
    Write-Host ""
    Write-Host "===========================================================" -ForegroundColor Cyan
    Write-Host "          Installation Summary & Validation" -ForegroundColor Cyan
    Write-Host "===========================================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0 -and $nodeVersion -match 'v(\d+\.\d+\.\d+)') {
            ...
            throw "Command failed or version not detected"
        }
    }
    catch {
        $validationResults['Node.js'] = @{
        ...
        $criticalFailures += "Node.js is required for Claude Code CLI"
    }

    # ===================================================================
    # Validate npm (comes with Node.js)
    # ===================================================================
    try {
        $npmVersion = npm --version 2>&1
        if ($LASTEXITCODE -eq 0 -and $npmVersion -match '\d+\.\d+\.\d+') {
            $validationResults['npm'] = @{
                Status = 'SUCCESS'
        ...
        $criticalFailures += "npm is required for installing packages"
    }
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0 -and $gitVersion -match 'git version ([\d\.]+)') {
        ...
        $criticalFailures += "Git is required for version control"
    }

    # ===================================================================
    # Validate Visual Studio Code
    # ===================================================================
    try {
        $vscodeVersion = code --version 2>&1 | Select-Object -First 1
        if ($LASTEXITCODE -eq 0 -and $vscodeVersion -match '\d+\.\d+\.\d+') {
            $validationResults['Visual Studio Code'] = @{
                Status = 'SUCCESS'
                Version = "v$vscodeVersion"
                Icon = '✓'
                Color = 'Green'
            }
        } else {
        ...
        $warnings += "VS Code may need a terminal restart to be available in PATH"
    }
    ...
    try {
        $claudeVersion = claude --version 2>&1
        if ($LASTEXITCODE -eq 0) {
            $validationResults['Claude Code CLI'] = @{
                Status = 'SUCCESS'
        ...
        $criticalFailures += "Claude Code CLI failed to install or is not working correctly"
    }

    # ===================================================================
    # Validate GitHub Desktop (Optional)
    # ===================================================================
    if ($installPieces) {
        $ghDesktopInstalled = Test-GitHubDesktop
        if ($ghDesktopInstalled) {
            $validationResults['GitHub Desktop'] = @{
                Status = 'SUCCESS'
                Version = 'Installed'
                Icon = '✓'
                Color = 'Green'
    ...
    # ===================================================================
    # Validate Pieces Desktop (Optional)
    # ===================================================================
    if ($installPieces) {
        $piecesCheck = winget list --id 9NB490VLC1LL 2>&1 | Out-String
        if ($piecesCheck -match "9NB490VLC1LL" -or $piecesCheck -match "Pieces") {
            $validationResults['Pieces Desktop'] = @{
                Status = 'SUCCESS'
                Version = 'Installed'
                Icon = '✓'
                Color = 'Green'
    ...
    # Display Results
    # ===================================================================
    Write-Host "Component Validation Results:" -ForegroundColor Cyan
    Write-Host ""

    # Display each component
    foreach ($component in $validationResults.Keys | Sort-Object) {
        ...
        $componentPadded = $component.PadRight(25)
        $versionPadded = $version.PadRight(20)

        Write-Host "  $icon $componentPadded $versionPadded" -ForegroundColor $color -NoNewline
    ...
    if ($criticalFailures.Count -gt 0) {
        ...
        Write-Host "===========================================================" -ForegroundColor Red
        Write-Host ""
        Write-Host "The following critical components are not working:" -ForegroundColor Red
        Write-Host ""
        ...
        Write-Host "  1. Restart your terminal or computer" -ForegroundColor Cyan
        Write-Host "  2. Re-run this installer" -ForegroundColor Cyan
        Write-Host "  3. Check installation log: $env:LOCALAPPDATA\ClaudeCodeInstaller\install-log.json" -ForegroundColor Cyan
        Write-Host "  4. Report issues at: https://github.com/anthropics/claude-code/issues" -ForegroundColor Cyan
        Write-Host ""
    }
    # ===================================================================
    # Display Warnings
    ...
    elseif ($warnings.Count -gt 0) {
        ...
        Write-Host ""
        Write-Host "[INFO] These are minor issues that can usually be resolved by:" -ForegroundColor Cyan
        Write-Host "       - Restarting your terminal" -ForegroundColor Cyan
        Write-Host "       - Logging out and back in" -ForegroundColor Cyan
        Write-Host ""
    }
    # ===================================================================
    ...
    # ===================================================================
        ...
        Write-Host "===========================================================" -ForegroundColor Green
        Write-Host ""
        Write-Host "Your Claude Code development environment is ready!" -ForegroundColor Green
        Write-Host ""
    }

    # Show original installation tracking (already vs newly installed)
    if ($alreadyInstalled.Count -gt 0) {
        Write-Host "Components already present before installation:" -ForegroundColor Cyan
    ...
    if ($newlyInstalled.Count -gt 0) {
        Write-Host "Components installed during this session:" -ForegroundColor Cyan
